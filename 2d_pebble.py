...
def Find_Pebble(v,seen,path,Pebbles):
    ...
    path[v]=-1
    # if the first pebble of v is un assigned 
    if(Pebbles[v][0]==None):
        return True # Return that a free pebble is found 
    if(Pebbles[v][1]==None):
        return True # the other pebble is free and hence return
    X=Pebbles[v][0] # X is the other node with which v has an edge and that edge is covered with the first pebble of v
    if( seen[X] == False ):
...
def Rearrange_Pebbles(v,v_,path,Pebbles):
    ...
    if(Pebbles[v][0]==None): # Let us just check if there is a free pebble and move on !
        Pebbles[v][0]=v_
        return 
    ...
    while(path[v]!=-1): # Start following the path 
        ...
        if(path[X] != -1 ): # If the node to which v points to is not the end of path 
            W=path[X]
            if(Pebbles[X][0]==W): # find the pebble which is used to cover w ( from x)
                Pebbles[X][0]=v
            ...
            if(Pebbles[X][0]==None): # this is free pebble
                Pebbles[X][0]=v
    ...
    if(Pebbles[v_copy][0]==path[v_copy]):
        Pebbles[v_copy][0]=v_
        return 
...
for label,ie in enumerate(independent_edges):
    ...
    if(RIGID_EDGES[ie[0]][ie[1]]== -1):
        ...
        #print rigid,label
with open('redundant_edges_'+(sys.argv[2]),"w") as filename:
    for e in redundant_edges:
        filename.write(str(e)+"\n")
with open('independent_edges'+(sys.argv[2]),"w") as filename:
    for e in independent_edges:
        filename.write(str(e)+"\n")
with open('statistics_FTN',"a") as filename:
    filename.write(sys.argv[2]+"\t"+str(len(independent_edges))+"\t"+str(len(redundant_edges))+"\t"+str(free_pebbles)+"\t"+str(max_size)+"\n")
##########################################################################################################
...
#CONFIG=np.loadtxt('conf')
#
#BOX=2*float(sys.argv[3])
#gamma=sys.argv[2]
#print BOX,gamma
...
#plt.savefig('network_'+(sys.argv[2])+'.pdf')
#########################################################################################################
###########################CODE TO DRAW CONFIGS##########################################################
################################END######################################################################
#########################################################################################################
